...
import { handleRenameLobbySelf } from "@/lib/main-room-engine";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const out = await mutateGameRoom(roomId, async (room) => {
      const r = handleRenameLobbySelf(room, playerId, body.displayName);
    ...
    console.error("POST /api/game/rename-display-name", e);
