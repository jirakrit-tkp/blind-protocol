...
import { handleSetHostLlm } from "@/lib/main-room-engine";
import { isSupabaseConfigured } from "@/lib/supabase/service";
import type { SetHostLlmBody } from "@/lib/host-llm-config";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const body = (await req.json()) as SetHostLlmBody;
    const out = await mutateGameRoom(roomId, async (room) => {
      const r = handleSetHostLlm(room, playerId, body);
    ...
    console.error("POST /api/game/host-llm", e);
